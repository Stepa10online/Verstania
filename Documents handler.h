    ...
    Passport& passport, Pass& pass, Papers Right, Image& image)
{
    ...
    for (auto& doc : docs)
    {
        ...
        if (doc.getDocID() == Document::Document_ID::Pass)
        {
            ...
            sprite.setScale(doc.getScale().x, 2);
        ...
        if (doc.getDocID() == Document::Document_ID::None/*rules*/)
        {
        ...
        if (doc.getDocID() == Document::Document_ID::Right)
        {
            texture.loadFromFile(doc.getTexture()[0]);
            sprite.setTexture(texture);
    ...
    for (auto& document : docs)
    {
        ...
        if (document.getDocID() == "passport")
        {
            ...
            if (doc.isAdditionalDataContains("opened") && Passport::getStamped())
